...
--ek ek kr add hoga jaise phle 750 hua phit next row me 750 aur next row 1250 dono add hoga ..
...
--asign the unique row number to each prouduct within  the same category 
select product_name , category ,price,
row_number() 
over(partition by category order by price)
from products;
